...
export async function GET(req: Request) {
  try {
    ...
    if (!canViewJourneys(resolved.roles)) {
      return noStoreJson(
        { success: false, error: "Trip evidence access required" },
        { status: 403 }
      );
    }
    ...
    if (relatedType !== "trip" || !relatedId) {
      return noStoreJson(
        { success: false, error: "relatedType=trip and relatedId are required" },
        { status: 400 }
      );
    }

    const trip = await loadTrip(resolved.company.id, relatedId);
    if (!trip) {
    ...
    return noStoreJson({
      ...
      related: {
        type: relatedType,
        id: relatedId,
        trip_reference: trip.internal_trip_id || null,
      },
      attachments,
      guardrails: {
    ...
    return noStoreJson(
      { success: false, error: err.message || "Failed to load trip evidence" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!canViewJourneys(resolved.roles)) {
      return noStoreJson(
        { success: false, error: "Trip evidence upload access required" },
        { status: 403 }
      );
    }
    ...
    if (!ALLOWED_EVIDENCE_TYPES.has(evidenceType)) {
      return noStoreJson(
        { success: false, error: "Unsupported evidence type" },
        { status: 400 }
      );
    }
    ...
    const storagePath = `${resolved.company.id}/trips/${relatedId}/${attachmentId}-${originalFilename}`;
    ...
    return noStoreJson(
      {
        ...
        message: "Evidence uploaded and attached to the Trip.",
      },
      { status: 201 }
    );
  } catch (err: any) {
    ...
    return noStoreJson(
      { success: false, error: err.message || "Failed to upload trip evidence" },
      { status: 500 }
    );
  }
}
  ...
  return noStoreJson(
    {
      success: false,
      setup_required: true,
      error: "Trip evidence table is not available yet. Apply the evidence_attachments migration first.",
    },
    { status: 424 }
  );
}
  ...
  return noStoreJson(
    {
      ...
      setup_required: true,
      error:
        "Trip evidence storage bucket is not available yet. Create a private trip-evidence bucket or apply the storage migration.",
    },
    { status: 424 }
  );
}
