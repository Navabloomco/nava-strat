...
import { buildOperationalEfficiencySummary } from "../../../../lib/intelligence/operationalEfficiency";
...
export async function GET(req: Request) {
  try {
    ...
    if (!canViewOps(resolved.roles)) {
      return noStoreJson(
        { success: false, error: "Operations access required" },
        { status: 403 }
      );
    }
    ...
    console.error("Ops efficiency error:", err);
    return noStoreJson(
      { success: false, error: err.message || "Failed to load operational efficiency" },
      { status: 500 }
    );
  }
}
