...
| `/ops/efficiency` | Ops Intelligence and Trip Intelligence dashboard. Uses authenticated app session fetches to call `/api/ops/efficiency` and `/api/ops/trip-intelligence`, showing movement, stopped-time evidence, tracker idle markers, stale-location, low productive-time, trip-readiness, missing-data, and evidence-source summaries without raw coordinates, engine-on idle claims, or unsafe fuel/contribution conclusions. |
