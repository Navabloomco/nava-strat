...
| API Route | Purpose |
| --- | --- |
| `GET/PATCH /api/finance/revenue` | Revenue/rates/quantity workflow. Finance visibility/edit gates apply. PATCH still updates compatibility revenue snapshots on `journeys` and now also writes a `manual_finance_entry` / `overridden` audit record to `journey_revenue_entries` when the revenue-entry migration is present. |
| `GET/POST /api/finance/rate-rules` | Client Rates / Revenue Rules Phase 1 backend. Finance/management/elevated roles may list Finance-owned client/route rate rules; finance/elevated roles may create them. Rules are company-scoped and include client name, optional route, unit type, billing quantity source, rate, currency, FX policy, effective period, status, and notes. |
...
| `client_rate_rules` | Client Rates / Revenue Rules Phase 1 table. Finance-owned company-scoped rules describe client name, optional route, unit type, billing quantity source, rate amount, currency, FX policy, effective window, status, and notes. Ops/clerk users should not need to know or see these rates. |
