...
Provider Vault cards should lead with a customer-ready status summary: connection status, last test result, vehicles found, matched/unmatched trucks, live tracking verification, engine/fuel verification, report/distance feed status, and the next action. Metric placeholders should use plain status language such as `Not refreshed yet` or `Needs review`, not command labels such as `Run test`. Credential fields belong under a collapsed Manage credentials section, while technical panels such as provider connection contract, row paths, response shape, data discovery, enrichment diagnostics, and CSV fallback/backfill import belong under a collapsed Advanced diagnostics section. Customer admins may expand safe diagnostics when allowed; platform owners retain deeper troubleshooting visibility. No secrets, tokens, raw payloads, cookies, passwords, or full coordinates may appear in either view.
