...
| `GET/POST /api/evidence` | Trip Evidence Attachments Phase 1. Company-scoped, authenticated evidence/receipt metadata and server-mediated private file upload for `relatedType=trip`. GET returns safe attachment metadata and short-lived signed URLs; POST validates the same-company Trip, evidence type, file type/size, uploads to private Supabase Storage, and stores metadata in `evidence_attachments`. No public files, cross-company access, raw storage paths, or M-Pesa parsing. |
...
| `evidence_attachments` | Trip Evidence Attachments Phase 1 metadata for receipts, M-Pesa screenshots, delivery notes, weighbridge tickets, invoices, and other trip documents. Files live in a private Supabase Storage bucket such as `trip-evidence`; the table stores company, Trip relationship, type, filename, MIME, size, notes, verification status, uploader UUID, and upload timestamp only. |
