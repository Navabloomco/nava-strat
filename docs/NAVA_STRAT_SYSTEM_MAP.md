...
Custom API provider setup supports safe auth options: no auth/public endpoint, API key header, bearer token, basic username/password, and POST login token where the current provider test infrastructure can authenticate and safely extract a token. The default Simple Connect surface asks for provider name, API link/base URL, email or username, and password/API key, then runs a visible setup checklist before creating the connection inactive. The advanced setup form remains collapsed for endpoint, row path, mapping, and signal capability review. The wizard normalizes base API URLs, splits endpoint-specific values such as `/login` or `/get_devices` back to the provider API base, fills common login/fleet endpoint candidates, and auto-tests common token and vehicle-row patterns such as `user_api_hash`, bearer token, API-key header, and row arrays like `data`, `items`, `devices`, or `vehicles`. If provider notes or URLs look like a `user_api_hash` / `get_devices` API, the guided setup should offer a one-click POST-login-token pattern instead of making a non-technical admin combine token paths manually. Customer-submitted request bodies must be JSON objects and must not contain credential-like keys; credentials belong in secure auth fields and are never echoed back to customers after save.
