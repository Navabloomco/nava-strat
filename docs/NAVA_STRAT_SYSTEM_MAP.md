...
| API Route | Purpose |
| --- | --- |
| `GET/POST /api/journeys` | Company-scoped journey list/create with role-gated finance fields. New production trip creation sets `is_demo = false`, stores selected `asset_id` / `driver_id` when provided, preserves `start_time` / `end_time`, and accepts optional rate/tonnage fields without requiring fuel. |
