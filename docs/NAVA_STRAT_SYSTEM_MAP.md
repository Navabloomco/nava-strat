...
| `/tracking/live` | Live tracking view for enabled intelligence assets. Top cards show operational state (`Enabled assets`, `Live now`, `Stale assets`, `Telemetry 24h`, `Providers`); provider/raw import counts stay out of the primary customer-facing metric row. |
...
| `/ops/journey/[id]` | Trip Detail Phase 1. Opens one production Trip, shows Trip Intelligence readiness, missing-data notes, management flags, linked fuel allocations, linked expenses, and role-gated edit forms for driver/timing, revenue, fuel allocation, and expenses. Fuel is allocation evidence only, not burn/theft proof. |
