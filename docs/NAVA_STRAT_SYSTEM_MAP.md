...
| `/admin/team-access` | Controlled-pilot Team Access for owner/admin/platform-owner roles. Tenant admins see only tenant-manageable `company_users` memberships enriched with safe auth email/name metadata; internal platform-owner memberships are hidden from tenant rows and counts. Admins can invite users by email, add/reactivate existing authenticated users, change pilot roles, revoke/resend pending invites, and deactivate/reactivate users. Auth invite emails are sent only by trusted server code using Supabase admin auth. Platform owners manage access only in an explicit company context and may see internal rows for support. |
...
| `company_user_invitations` | Company-scoped Team Access invitations. Stores invited email, role, status, inviter, accepted/revoked timestamps, optional Supabase user id, and a safe invite-error summary. No service-role keys, invite tokens, or raw Supabase admin errors are stored for browser display. |
