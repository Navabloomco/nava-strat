...
| `GET /api/ops/trip-intelligence` | Role-gated Trip Intelligence Phase 1 JSON foundation. Projects existing `journeys` into business-readable Trip records with asset, driver, movement, delay, finance-readiness, contribution-review readiness, and management flags. Uses current tables only, does not add schema, does not require fuel, and hides finance amounts from roles without finance visibility. |
