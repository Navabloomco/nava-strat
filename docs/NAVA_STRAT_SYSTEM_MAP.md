...
- Trips, fuel, expenses, revenue review, and review-ready contribution.
...
- Platform-owner readiness and provider onboarding tools.
...
Nava Strat should sound like a mature enterprise SaaS: operational, concise, source-grounded, and role-aware. Customer-facing pages should lead with clear next actions and business language, while advanced implementation details belong in collapsed diagnostics, docs, or Nava Eye audit mode.

- Use `Trip`, `Ops Intelligence`, `Revenue Review`, `Client rates`, `Contribution review`, and `review-ready contribution` in UI copy. Do not use `profit` unless the system has final audited profit evidence.
- Keep evidence caveats, but make them compact: `GPS-derived distance estimate`, `provider distance evidence`, `stopped-time evidence`, `tracker idle markers`, and `engine-on idle not verified` are preferred user-facing labels.
...
- Operations pages may show Trip status, distance evidence, stopped-time, tracker idle markers, availability context, missing operational links, proof status, Trip expense amount/transaction fee capture, and finance/contribution review readiness. Operations pages must not render revenue amounts, linked cost amounts, contribution amounts, margin, per-km money metrics, fuel cost amounts, or rate details as the default UI.
- Finance owns rates, revenue, expense validation/correction/allocation, transaction fee treatment, fuel cost review, and contribution review.
...
| Route | Purpose |
| --- | --- |
| `/dashboard` | Customer-facing Command Brief for fleet tenants. It leads with role-aware needs-attention action cards, compact fleet-state tiles, review queues, secondary command routes to Live Tracking, Trips, Ops Intelligence, Revenue Review, Provider Vault, and Nava Eye, then grouped event review and saved intelligence notes. It should summarize what changed, what is blocked, and where to act next instead of duplicating specialist pages. The top Nava Eye block can ask about the current brief and shows a few tenant-safe suggested questions; embedded answers render directly under that top prompt and scroll/focus there when triggered from lower dashboard action cards or watch items. Platform owners default to the Nava Bloom Co./Navabloomco platform operator workspace on first load. Durable detection uses `companies.company_type = platform_operator`, with the older Navabloomco slug/name heuristic only as a transition fallback when `company_type` is missing. For the platform/operator workspace, platform owners see a platform workspace home with safe aggregate KPIs, grouped platform/tenant/product actions, customer workspace cards, and a presentation-only sensitive metric toggle instead of empty fleet metrics. Customer tenants remain selectable from the company switcher. Includes Nava Eye Watch items built from safe dashboard summaries, and an embedded Nava Eye widget for customer fleet tenants that may pass safe page context for visible dashboard follow-ups. |
| `/nava-eye` | Nava Eye assistant UI with company-scoped investigation conversation threads. The page is the flagship private AI workspace: a slim conversation rail, a wide central message surface, compact contextual prompt chips, and a bottom composer that stays easy to reach on desktop and mobile. Threads are separate from durable Nava Eye memory and can be closed when the investigation is handled. Closing a conversation clears the active thread and returns the page to a safe new/list state rather than silently selecting another thread. Users can also delete their own conversations from the visible list through a confirmed soft-delete action. The page preserves selected company, selected conversation, and open/closed tab through URL query state on ordinary refresh without storing message content in localStorage. Contextual entry points across the app may open `/nava-eye` with a scoped prompt and company context; the page consumes that prompt, creates a fresh investigation thread, clears the prompt from URL state, and then relies on the normal server-side role/company checks. Closed conversations are archived/read-only for MVP and remain accessible under Closed conversations unless the user deletes them from their own list; future retention can become configurable, such as 90 days. |
| `/tracking/live` | Live tracking view for enabled intelligence assets. Top cards show operational state (`Enabled assets`, `Live now`, `Stale assets`, `Telemetry 24h`, `Providers`); provider/raw import counts stay out of the primary customer-facing metric row. A compact command bar filters the company-scoped live/stale rows by truck, safe provider label, trailer, provider, client/route, location, geofence, and active Trip context without exposing raw coordinates. Search/filter result sets and individual live/stale rows provide compact `Ask Nava Eye` entry points that pass safe truck/result context only. Live/stale rows may show active Trip context from company-scoped non-demo `journeys`, but Trip route context stays separate from current provider location and is not proof of arrival/offload. When ops/admin users record a lightweight asset availability status, rows show a compact status chip such as `Under repair`, `Grounded`, `At client/site`, `Loading`, or `Waiting` as operational context, not as an incident workflow. |
...
| `/ops/efficiency` | Ops Intelligence and Trip Intelligence dashboard. Uses authenticated app session fetches to call `/api/ops/efficiency` and `/api/ops/trip-intelligence`, showing movement, stopped-time evidence, tracker idle markers, stale-location, low movement review, trip-readiness, missing-data, and evidence-source summaries without raw coordinates, engine-on idle claims, or unsafe fuel/contribution conclusions. It may show finance/contribution review status and missing finance links, but not revenue, linked cost, contribution, margin, per-km money, fuel cost, or rate amounts. Lightweight asset availability status is shown as interpretation context so grounded, repair, breakdown, or out-of-service assets are not treated as ordinary low-movement candidates. Key rows include compact Nava Eye entry points for distance evidence, stopped context, idle-marker interpretation, low movement next steps, known unavailable status, and Trip review blockers. |
| `/ops/journey` | Trip list with create CTA and trip cards/table. |
...
| `/ops/journey/[id]` | Trip Detail Phase 1. Opens one production Trip and separates operational entry from finance/management intelligence. Clerks/ops can complete allowed trip status, driver, timing, trip expenses, transaction fees shown on receipts/payment messages, expense proof, general trip evidence, and lightweight asset availability context for the Trip truck. Finance/management/elevated roles see revenue, rates, contribution summary, margin, and management flags according to role gates. A compact Nava Eye context strip can ask about the current Trip, missing links, expenses/proof, and finance/contribution review only where the role allows it. Fuel is allocation evidence only, not burn/theft proof. |
...
| `/expenses` | Finance-gated Expense ledger showing expense amount, transaction fee, total paid, proof/review context, and Trip allocation. |
| `/expenses/new` | Expense entry form with JourneyPicker, payment reference, optional transaction fee, and Trip allocation. |
...
| `/admin/providers` | Provider Vault for tracking provider configuration, testing, sync diagnostics, activation, and enrichment diagnostics. Supports platform-owner `?companyId=` tenant context. Provider-level Nava Eye entry points use sanitized provider capability summaries and setup state, never raw payloads, secrets, or response samples. |
...
| `/admin/team-access` | Controlled-pilot Team Access for owner/admin/platform-owner roles. Tenant admins see only tenant-manageable `company_users` memberships enriched with safe auth email/name metadata; internal platform-owner memberships are hidden from tenant rows and counts. Admins can invite users by email, add/reactivate existing authenticated users, change pilot roles, revoke/resend pending invites, and deactivate/reactivate users. Initial invite emails are sent only by trusted server code using Supabase admin auth; resend for an existing unverified Auth user uses the verification-email path instead of creating a duplicate invite. Platform owners manage access only in an explicit company context and may see internal rows for support. |
...
| `app/components/NavaEyePromptLink.tsx` | Reusable compact contextual entry point that opens Nava Eye with a scoped prompt, optional company context, and optional record context. The server-side Nava Eye route remains responsible for enforcing company and role permissions. |
...
| `GET/POST/PATCH /api/admin/team-access` | Owner/admin/platform-owner Team Access API for controlled pilots. Lists tenant-manageable company memberships and pending invites, sends real Supabase email invitations from trusted server code, resends pending verification email for existing unverified Auth users, adds/reactivates verified existing users, changes pilot roles, revokes/resends pending invites, and deactivates/reactivates access. It is company-scoped, blocks non-admin writes, hides platform-owner rows from tenant admins, rejects tenant attempts to edit internal rows without naming internal access mechanics, prevents removing the last active owner/admin, and never exposes service-role credentials. |
...
| `GET /api/admin/pilot-readiness/[companyId]` | Platform-owner-only tenant readiness detail grouped by readiness category. No mutation. |
...
| `GET/POST/PATCH /api/asset-availability` | Asset Availability Status Phase 1. Company-scoped, authenticated operational context for enabled or known trucks/assets. GET returns active lightweight availability status by asset/truck or all active statuses for the company. POST lets ops/admin-style roles mark one active status such as `grounded`, `under_repair`, `breakdown_reported`, `out_of_service`, `at_client_site`, `loading`, `offloading`, `waiting`, or `unknown_stopped_time`; posting `available` clears the active status. PATCH ends a current status. This is manual operating context only, not a maintenance, transfer, repair-vendor, or incident module. |
...
| `GET /api/ops/efficiency` | Role-gated operational efficiency JSON foundation for current provider assets. Returns company-scoped movement, GPS-stopped evidence, provider idle-marker evidence, stale-location, productivity, driver-readiness, and client-waiting readiness summaries for today, yesterday, or 7-day windows. No UI, no fuel/contribution math, no raw coordinates, and no engine-on idle claim unless ignition/engine/CAN evidence supports it. |
| `GET /api/ops/trip-intelligence` | Role-gated Trip Intelligence Phase 1 JSON foundation for the Operations surface. Projects existing `journeys` into business-readable Trip records with asset, driver, movement, delay, finance-readiness, contribution-review readiness, and management flags. Uses current tables only, does not add schema, does not require fuel, and applies the Operations product boundary: safe review status may be returned, but revenue, linked cost, contribution, margin, per-km money, fuel cost, and rate amounts are hidden even for users who can view those values on Finance or Management surfaces. |
...
| `GET/POST /api/expenses` | Expense ledger and creation. Finance role gates apply for the general ledger. Trip-linked expense creation is also allowed for users who can edit journeys so clerks/ops can capture operational trip costs, transaction fees, payment references, and proof without seeing finance intelligence. |
| `GET /api/management/dashboard` | Role-gated Management Intelligence data. Requires finance/management/elevated visibility and supports `range=today|yesterday|7d|30d`. Returns Trip Intelligence-backed review-ready contribution, contribution per active day, Trip cycle/duration metrics, client contribution velocity, trips needing review, and delay/operational-drag categories without raw coordinates or final audited profit claims. |
...
| `GET/PATCH/DELETE /api/nava-eye/conversations/[id]` | Reads one current-user conversation with messages, closes an open conversation, or soft-deletes the current user's conversation from their visible list. Closed conversations are read-only in the MVP. Delete is still company/user-scoped and requires the `deleted_at`/`deleted_by` conversation migration. |
...
| `company_user_invitations` | Company-scoped Team Access invitations. Stores invited email, role, status, inviter, accepted/revoked timestamps, optional Supabase user id, and a safe invite-error category such as `existing_auth_user`, `smtp_failure`, `redirect_failure`, `rate_limited`, or `unknown`. No service-role keys, invite tokens, or raw Supabase admin errors are stored for browser display. |
| `nava_eye_memory` | Nava Eye memory and recurring operational facts. |
| `nava_eye_conversations` | Company-scoped, user-started Nava Eye investigation threads. Open/closed status, last intent, short pending follow-up context, and optional soft-delete metadata only. |
...
| `asset_availability_events` | Lightweight Asset Availability Status Phase 1. Stores company-scoped active or ended manual/provider/inferred availability context for one asset or truck, optionally linked to a Trip. Supported statuses include `available`, `on_trip`, `grounded`, `under_repair`, `breakdown_reported`, `out_of_service`, `at_client_site`, `loading`, `offloading`, `waiting`, and `unknown_stopped_time`. Live Tracking, Ops Intelligence, Trip Detail, and Nava Eye may show the latest active status as operating context for stopped-time interpretation. It is not a full maintenance, incident, supplier, repair-vendor, or cargo-transfer model. |
...
| `expenses` | Expense ledger records linked to journeys when available. Phase 1 structured transaction details live here where possible: supplier/vendor/payee (`vendor`), payment method, reference number, base expense amount (`amount`), transaction fee/cost (`transaction_cost`, default 0), and transaction/date evidence from existing date fields such as `created_at`. App/API responses compute `total_paid = amount + transaction_cost`; there is no stored raw-token or provider payload data in expense records. |
...
Team Access Phase 1 uses `company_users` as the active access source and `company_user_invitations` as the pending invite ledger. Owner/admin/platform-owner users can manage company memberships from `/admin/team-access`, with tenant-editable roles limited to `owner`, `admin`, `ops`, `finance`, and `management`; `platform_owner` cannot be invited or granted from tenant Team Access. Tenant admins must not see platform-owner/internal support memberships, labels, or counts in Team Access. First-time invites for emails without an Auth user are sent through `supabase.auth.admin.inviteUserByEmail` only inside server routes using the service-role client. Resending a pending invite for an existing unverified Supabase Auth user uses Supabase Auth's signup verification resend path; `generateLink` is not treated as an email sender because it only returns a link for systems with their own mailer. The invite redirect resolves from `NEXT_PUBLIC_SITE_URL` when set, otherwise the request origin, and should land on `/login` so the normal authenticated company resolution can run. Verified existing authenticated users can be added or reactivated immediately. Existing unverified Auth users remain pending until they verify/sign in and `/api/companies` or onboarding accepts the pending invitation server-side. New invited users become active company members only after they accept/sign in and the pending invitation is accepted server-side. Inactive memberships are excluded from `/api/companies` active company context and should not authorize workspace access. Platform owners must act in an explicit company context and should not use cross-company fallback for member edits.
...
Nava Eye trip-performance questions such as "How did this Trip perform?", "Did this Trip make money?", "What was the contribution on this Trip?", or "Is this Trip ready for contribution review?" must route to Trip Intelligence before live vehicle status. Matching should use internal trip reference, truck plus client text, truck plus route/destination text, or a clear recent trip for that truck. Exact trip reference wins; truck-plus-client, truck-plus-route, client-only, and truck-only matches may answer directly only when one production Trip matches the selected range. If multiple Trips match, Nava Eye must list safe candidates and ask the user to choose instead of falling back to live truck status or silently guessing. Finance-visible roles may see revenue, allocated fuel cost, linked expenses, contribution, margin, and per-tonne/per-km values where available. Roles without finance visibility should still receive a useful trip readiness answer with amount fields hidden: trip name, readiness label, revenue-present/fuel-allocation-linked/expense-linked status, missing distance or extra-expense caveats, and a finance-role boundary for contribution amounts.

Nava Eye query understanding lives in `lib/intelligence/queryUnderstanding.ts` and runs before source routing. It preserves the original user text, creates a normalized text form, records shorthand replacements, detects entity hints, periods, metric, intent family, follow-up status, answer mode, and whether the question is explicitly fleet-scoped or should inherit the active subject, then hands a structured intent to the conversation resolver and context router. It normalizes casual language such as `btwn` to between, `diff` to difference, `yday` to yesterday, `tdy` to today, `day b4 yesterday` to day before yesterday, `km/kms` to kilometers, and maps phrases like "how far", "gone", "covered", "stuck", "show proof", "made money", "so which is it", and "what should I do" into broad intent families instead of one-off route patches. It must not mutate vehicle plates, trip references, provider IDs, UUIDs, route text, or client names in ways that break entity matching. Structured intent families include distance, compare metric, explain previous answer, live status, truck day story, idle/stopped evidence, Trip performance, expense evidence, finance revenue review, provider capability, management actions, and unknown. The parser itself does not fetch private data or authorize answers; the server-side role/company gates still run before any answer.

Truck Day Story / daily activity summaries live in `lib/intelligence/truckDayStoryService.ts` and are used by Nava Eye for questions such as "what has this truck been up to today?", "what did this truck do today?", "movement summary today", and "show today's timeline". The service combines same-company enabled-asset identity, `truckTimelineService` movement/stationary blocks, Ops Intelligence distance evidence, GPS-stopped windows, provider current-stop evidence, tracker idle markers, active Trip context, and lightweight asset availability status into one structured source object. Default answers stay concise: start/current location, distance evidence, stopped-time summary, key stop, active Trip/availability context, and one next action. Timeline mode lists key blocks/stops without raw coordinates. Audit mode shows source hierarchy, telemetry point/block counts, first/last evidence timestamps, distance source, stopped-window method, missing evidence, and the provider-current-stop versus daily GPS-stopped distinction. It must not claim engine-on idle, fuel burn, theft, driver blame, client blame, or finance outcomes unless the specific role-safe evidence supports them.

Nava Eye conversation resolution Phase 1 lives in `lib/intelligence/conversationResolver.ts` and uses database-backed `nava_eye_conversations.pending_followup`, not process memory. The durable memory spine lives in `lib/intelligence/navaEyeContextSnapshot.ts`: after scoped answers, the copilot route stores a sanitized `context_snapshot` in the conversation `pending_followup` JSON and assistant-message metadata, then rebuilds active subject, last intent, last period, audit target, timeline target, comparison, and recent subject history from that persisted data when a serverless request starts cold. It does not store raw coordinates, provider payloads, signed evidence URLs, secrets, or unrestricted finance details. It resolves current-prompt truck/asset tokens before cached context, supports exact compact and unique-prefix asset matching, suggests close matches instead of silently choosing weak fuzzy matches, persists close-match clarification state, supports numbered/ordinal candidate replies where multiple true candidates remain, and reruns the original intent when the user confirms with "yes", "yeah", "correct", or "that truck". Duplicate-looking vehicle candidates must be collapsed by normalized provider label, truck/registration, and trailer context; enabled intelligence assets should win over matching unenabled/imported provider rows, and any remaining clarification candidates need distinguishing context. It inherits sensible active intents for "how about" / "what about" follow-ups, preserves fleet scope when explicitly requested, keeps distance/metric period follow-ups such as "what about yesterday?" or "what about yday?" on the prior metric instead of drifting into timeline mode, supports distance comparisons such as "difference in distance between yesterday and today" and "difference between yesterday and the day before yesterday" for the current truck/fleet subject, and treats conflict follow-ups such as "so which is it?" or "that doesn't make sense" as an audit/explanation of the previous answer instead of falling into fleet health. Follow-up comparisons must keep the active truck unless the user explicitly asks for fleet/all-truck/company total scope. Broad management prompts such as "what should I do today?" route to company-scoped action summaries, while subject-specific follow-ups such as "what should I do about it?" inherit the current truck or Trip. It supports audit follow-ups such as "how did you calculate that?" or "why so low?", and can keep a recent Trip topic so expense/evidence follow-ups such as "which expenses on this trip are missing proof?" attach to the same production Trip. `start over`, `clear context`, or equivalent commands write a durable clear marker with `context_cleared_at` and `context_generation`; context reconstruction, message metadata, pending clarifications, subject history, and action-planner recent-message inference must ignore anything before that marker so pronoun-only follow-ups ask for the subject again. Phase 1 deliberately covers trucks/assets, fleet, metric result references, active Trip evidence/performance follow-ups, active intent, and timeframes only; broader driver/client/route/provider conversations remain incremental. Nava Eye's copilot route is a source-grounded internal orchestration layer in Phase 1; external LLM calls must stay disabled unless a future security-reviewed private model path explicitly opts in.

Nava Eye source-grounded domain routing now has lightweight deterministic branches for expense/evidence proof review, Finance Revenue Review summaries, Provider Vault capability summaries, provider idle-marker fleet questions, Trip Intelligence performance answers, live tracking status, operational distance metrics, and action planning. Each branch must enforce company scope and role/capability gates before returning data. Evidence answers may show safe attachment/proof counts and private-record status, but must not expose public URLs, raw storage paths, or files unless the secure signed-link route authorizes them. Provider capability answers use Provider Vault safe test summaries and must say detected/mapped/not detected; field detection alone is not proof of fuel burn, theft, diagnostics, or true engine-on idle. Action planning lives in `lib/intelligence/navaEyeActionPlanner.ts`: broad prompts such as "What should I do today?" return prioritized company-scoped actions from role-safe fleet health and Trip review signals, while subject follow-ups such as "what should I do about it?", "what now?", or a bare "what should I do?" after a truck/Trip answer stay scoped to the active truck or Trip. The planner should recommend operational next checks, evidence completion, provider sync/freshness, distance-source review, finance escalation where permitted, and always hide restricted finance amounts from non-finance roles.
...
Truck conversation threads maintain a safe active truck topic plus the richer context snapshot in persisted conversation metadata. If a follow-up omits the truck ID, such as "what are yesterday's movements?", "show timeline", "where is it now?", "was it idling?", or "how did you calculate that?", Nava Eye should keep using the active truck and the latest audit/timeline target unless the user explicitly asks for fleet/all-truck scope or names a different truck. A detailed timeline follow-up should inherit the active truck timeline's resolved timeframe/date window unless the new message explicitly says today or yesterday. A truck ID written in the current prompt always wins over cached truck or fleet context and pushes the previous subject into short subject history. If no active truck topic exists for an elliptical truck question, ask which truck to check. Relative dates such as today/yesterday must resolve once in the company/operator timezone and stay consistent between truck summary, truck detail, and fleet movement summary modes.

Nava Eye conversations are short investigation threads, not durable operational memory. A user can access their own conversations in companies where they have active access; platform owners can create/use conversations in an explicitly resolved company context. Role/capability checks must run on every copilot message, so old conversation context must not unlock finance, billing, provider, or tenant data that the current role cannot see. Closed conversations are read-only in the MVP. User delete is a confirmed soft-delete from that user's visible list, not a cross-user transcript delete, and deleted threads must not stay selectable by direct URL when the soft-delete migration is applied. There is no admin transcript browser.

Asset availability status is a lightweight interpretation layer for operations, not a maintenance or incident system. Ops/admin-style roles can record a current status such as grounded, under repair, breakdown reported, out of service, at client/site, loading, offloading, waiting, or unknown stopped time. Nava uses the latest active status to reduce misleading stopped-time interpretation: known unavailable assets should be separated from ordinary low-movement candidates, while site/loading/waiting labels provide context without assigning client, driver, fuel, or engine-on idle blame. Breakdown incident details, replacement truck assignment, cargo transfer, repair-vendor records, and transfer evidence remain deferred workflows.
...
Provider-reported current-feed fields are evidence, not audited truth. FleetTrack/Oak-style current feeds may expose richer evidence such as `total_distance`, `engine_hours`, nested engine-hour, fuel quantity/price/rate, alarm, icon status fields, and current stop fields such as `stop_duration`, `stopDuration`, `stopped_since`, `parking_time`, or stop/status labels; BlueTrax-style current feeds may expose `mileage` and device timezone. Nava maps selected current-feed values into provider-reported evidence metadata so Operational Efficiency can prefer sane provider-reported distance/odometer deltas over GPS-estimated distance and reconcile provider current-stop episodes against Nava GPS-stopped selected-period totals. Fuel issued/allocation remains the finance/control source of record; provider fuel and stop/status fields do not create fuel-burn, theft, efficiency, driver-misuse, client-blame, or true engine-on idle claims. Richer historical distance, idle, diagnostics, HOS, or safety evidence may still require provider report/event/history endpoints and permissions.
