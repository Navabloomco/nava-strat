...
| `/ops/efficiency` | Ops Intelligence and Trip Intelligence dashboard. Uses authenticated app session fetches to call `/api/ops/efficiency` and `/api/ops/trip-intelligence`, showing movement, stopped-time evidence, tracker idle markers, stale-location, low productive-time, trip-readiness, missing-data, and evidence-source summaries without raw coordinates, engine-on idle claims, or unsafe fuel/contribution conclusions. Lightweight asset availability status is shown as interpretation context so grounded, repair, breakdown, or out-of-service assets are not treated as ordinary low-productivity candidates. Key rows include compact Nava Eye entry points for distance evidence, stopped context, idle-marker interpretation, low-productive-time next steps, known unavailable status, and Trip review blockers. |
