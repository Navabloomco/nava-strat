...
Nava Eye trip-performance questions such as "How did the KBJ132C Bamburi trip perform?", "Did the Bamburi trip make money?", or "Is this trip ready for profit review?" must route to Trip Intelligence before live vehicle status. Matching should use internal trip reference, truck plus client text, truck plus route/destination text, or a clear recent trip for that truck. Finance-visible roles may see revenue, allocated fuel cost, linked expenses, contribution, margin, and per-tonne/per-km values where available. Roles without finance visibility should still receive a useful trip readiness answer with amount fields hidden: trip name, readiness label, revenue-present/fuel-allocation-linked/expense-linked status, missing distance or extra-expense caveats, and a finance-role boundary for contribution amounts.
