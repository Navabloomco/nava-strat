...
5. Skip rows that do not have a safe vehicle identifier after mapped and fallback keys are checked. Provider sync must not create `UNKNOWN`, blank, or null reviewable assets.
   ...
   Asset Review counts, duplicate detection, and billable-review views use this canonical truck identity. Legacy rows that used a combined truck-and-trailer label are surfaced as possible duplicates of the canonical truck instead of counting as separate normal unreviewed assets.
