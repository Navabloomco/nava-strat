...
Customer-facing Trip Intelligence should not show the raw machine status `calculable` by itself. When revenue and at least one linked cost source such as a fuel allocation or trip expense exist, the UI label is "Contribution review ready." Distance remains a separate evidence requirement for per-km metrics, so missing distance should appear as "Distance evidence missing" / "Distance-based metrics pending" rather than making basic contribution review sound impossible. If fuel allocation exists but no other trip expenses are linked, show "No additional trip expenses linked yet" as a supporting note, not as a blocker to linked revenue-minus-cost contribution review. Exact profit completeness still requires the remaining links to be reviewed.
